...
export function GuestAuthorPage() {
  ...
  if (authorLoading || !author) {
    return <div className="text-center py-8 fuwari-text-50">加载中...</div>;
  }
      ...
      <div className="fuwari-card-base p-6">
        ...
        <div className="flex items-center gap-4">
          <img
            src={avatarSrc}
            alt={author.name}
            className="w-16 h-16 rounded-full object-cover border border-(--fuwari-primary)/20"
          />
          <div className="flex-1">
            <h1 className="text-2xl font-bold fuwari-text-90">{author.name}</h1>
            {author.bio && (
              <p className="text-sm fuwari-text-50 mt-1">{author.bio}</p>
            )}
          </div>
