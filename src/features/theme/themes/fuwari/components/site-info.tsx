import { useSiteStats } from "@/hooks/use-site-stats";
import { blogConfig } from "@/blog.config";
import { m } from "@/paraglide/messages";
import { Skeleton } from "@/components/ui/skeleton";
import { Clock, FileText, PencilLine, Eye } from "lucide-react";

// 计算运行时长，返回格式：“X 年 Y 天” 或 “X 天”
function formatRuntime(startDate: string): string {
  const start = new Date(startDate);
  const now = new Date();
  const days = Math.floor((now.getTime() - start.getTime()) / 86400000);
  const years = Math.floor(days / 365);
  const remainingDays = days % 365;
  if (years > 0) {
    return `${years} ${m.stats_years()} ${remainingDays} ${m.stats_days()}`;
  }
  return `${days} ${m.stats_days()}`;
}

export function SiteInfo() {
  const { data, isLoading } = useSiteStats();

  return (
    <div className="fuwari-card-base rounded-(--fuwari-radius-large) p-6">
      {/* 标题 */}
      <h2 className="text-xl font-bold mb-4 fuwari-text-90">
        {m.site_info_title()}
      </h2>

      {/* 数据列表 */}
      <div className="space-y-3 text-sm">
        {isLoading ? (
          // 骨架屏
          <>
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex items-center gap-3">
                <Skeleton className="h-4 w-4 rounded" />
                <Skeleton className="h-4 w-3/5 rounded" />
              </div>
            ))}
          </>
        ) : data ? (
          <>
            {/* 运行时长 */}
            <div className="flex items-center gap-3 fuwari-text-70">
              <Clock size={16} className="shrink-0 text-(--fuwari-primary)" />
              <span>
                {m.stats_runtime()}: {formatRuntime(data.startDate)}
              </span>
            </div>

            {/* 文章数量 */}
            <div className="flex items-center gap-3 fuwari-text-70">
              <FileText size={16} className="shrink-0 text-(--fuwari-primary)" />
              <span>
                {m.stats_articles()}: {data.articleCount.toLocaleString()}
              </span>
            </div>

            {/* 总字数 */}
            <div className="flex items-center gap-3 fuwari-text-70">
              <PencilLine size={16} className="shrink-0 text-(--fuwari-primary)" />
              <span>
                {m.stats_total_words()}: {data.totalChars.toLocaleString()}
              </span>
            </div>

            {/* 全站浏览量 */}
            <div className="flex items-center gap-3 fuwari-text-70">
              <Eye size={16} className="shrink-0 text-(--fuwari-primary)" />
              <span>
                {m.stats_pageviews()}: {data.totalPageviews.toLocaleString()}
              </span>
            </div>
          </>
        ) : null}
      </div>
    </div>
  );
}
